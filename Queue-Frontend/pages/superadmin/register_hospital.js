...
export default function RegisterHospitalBySAdmin() {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (response.data.message == "Registration failed") {
                ...
                setSuccess("");
                setError(<span>An admin with the same ID already exists</span>);
            } else {
                setError("");
                        ...
                        <div className="relative flex items-center justify-center rounded-md bg-white shadow-md p-8 w-80 h-fit">
                            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                                ...
                                <div className="flex flex-col w-full justify-center mb-4">
                                    ...
                                    />
                                    {errors.adminid && (
                                        <p className="font-semibold text-center text-red-700 text-[12px] leading-4 pb-2">Admin ID is required</p>
                                    )}
                                </div>
